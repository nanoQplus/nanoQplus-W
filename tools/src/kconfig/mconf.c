...
static void cprint_init(void)
{
	...
	env = getenv("NOS_HOME");
	cprint("%s/tools/lxdialog.exe", env);
	cprint("--backtitle");
...
static void build_conf(struct menu *menu)
{
	...
	if (sym_is_choice(sym)) {
		...
		if (menu == current_menu) {
			cprint(":%p", menu);
			cprint("-o-%*c%s", indent + 1, ' ', menu_get_prompt(menu));
			goto conf_childs;
		}
		child_count++;
		val = sym_get_tristate_value(sym);
...
int main(int ac, char **av)
{
	...
	sprintf(menu_backtitle, "Nano OS Kernel Configuration");
	...
	if (stat == 0) {
		...
		printf("\n"
			"*** End of Nano OS kernel configuration.\n"
			"*** Execute 'make' to build the application."
			"\n\n");
	} else {
		fprintf(stderr, "\n\n"
			"Your kernel configuration changes were NOT saved."
